...
vector<vector<int>> multiplyMatrices(const vector<vector
...
vector<vector<int>> matrixPower(vector<vector
...
int main() {
  ...
  /*  if (n < 5 || m < 5) {
    ...
    vector<vector<int>> poweredMatrix =
        matrixPower(adjacencyMatrix, pathLength);

    cout << "\nМатрица смежности возведенная в степень: " << endl;
    ...
    cout << "Общее количество путей длины " <<
        pathLength << ": " << totalPaths << endl;
}